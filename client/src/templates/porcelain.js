export default {
  id: "porcelain",
  name: "Delft",
  description: "Blue-and-white ceramic elegance. Delicate double border, refined serif type.",
  category: "classic",
  previewColors: ["#FFFFFF", "#2B5BA3", "#1A2744"],
  starterSections: ["Appetizers", "Seafood", "Mains", "Sides", "Desserts"],
  colors: {
    bg: "#FFFFFF",
    text: "#1A2744",
    accent: "#2B5BA3",
    muted: "#7889A0",
    divider: "#2B5BA333",
    heading: "#2B5BA3",
  },
  fonts: {
    title: "'Crimson Pro', serif",
    heading: "'Crimson Pro', serif",
    body: "'Crimson Pro', serif",
    price: "'Crimson Pro', serif",
    imports: [
      "Crimson+Pro:ital,wght@0,300;0,400;0,500;0,600;0,700;1,400",
    ],
  },
  sizes: { title: 34, subtitle: 12, section: 15, dish: 15, desc: 12, price: 15 },
  spacing: { sectionGap: 30, dishGap: 14, pagePadding: 46, headerBottom: 36, descTop: 3 },
  typography: {
    titleWeight: 600,
    titleLetterSpacing: "3px",
    titleTransform: "uppercase",
    sectionWeight: 500,
    sectionLetterSpacing: "3px",
    sectionTransform: "uppercase",
    dishWeight: 400,
    descStyle: "italic",
  },
  layout: {
    dotLeader: true,
    dotChar: "·",
    dotColor: "#2B5BA344",
    dividerStyle: "line-through",
    headerDecor: "dots-and-rule",
    border: "double 3px #2B5BA344",
    outerBorder: "none",
    borderInset: 6,
    priceAlignment: "right",
    sectionAlignment: "center",
  },
  logo: { width: 80, height: 80, shape: "rect" },
  allergens: { iconSize: 12, iconColor: "#7889A0", position: "after-desc" },
};
